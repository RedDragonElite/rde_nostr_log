...
**🔴 Fixes**
- **Duplicate player event logging eliminated** — Events were being posted twice (once from `server.js`, once from `player_activity.lua`). Player events now handled exclusively in Lua.
- **`DevMode` set to `false`** — Was hardcoded `true` in production, causing verbose console output
- **Private key hex no longer printed** to console on startup

**🟡 Notes**
- `config.lua` is client/UI config only — server config lives in `server.js`
- Player events: single source of truth is `server/player_activity.lua`
